...
public class Static01 {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 클래스와 / 객체는 생성되는 시점이 달라서 메서드와 멤버변수를 사용할 수 있는 환경이 다르다.(객체는 new키워드 있어야만 가능)
		 * 객체 멤버는 클래스가 생성되고 , 객체를 생성한 후 사용할 수 있음.
		 * 클래스 멤버는 클래스가 생성되면 바로 사용가능
		 * 
		 * -클래스 멤버는 클래스 멤버/ 객체 멤버에서 둘 다 사용가능.
		...
		//객체 멤버 호출
		TV t= new TV(); //이 시점에서 객체 멤버들이 생성
		t.printPower();
		t.printBrand(); //가능은 하지만 잘 쓰지 않음
		
	}
}
//Class 안에다 Class 만들면 안됨.
...
//		System.out.println(power);  //static 메서드에 일반 객체 멤버 변수는 호출 불가능
	}
